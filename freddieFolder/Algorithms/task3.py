...
def minimumSpeed(xf: float, yf: float, xi: float = 0, yi: float = 0):
    """
    This return the projectile object of the minimum speed object
    ...
    minAngle = math.degrees(math.atan((yf + math.sqrt(xf**2 + yf**2))/xf))
    return Projectile.Projectile(x=xi, y=yi, v=minSpeed, angle=minAngle)


def lowAndHighBall(xf: float, yf: float, initialVel: float, xi: float = 0, yi: float = 0):
    """
    :param xf: The desired x coordinate that it should go through
    :param yf: The desired y coordinate that it should go through
    ...
    lowBallAngle = math.degrees(math.atan((-b - math.sqrt(discriminant)) / (2 * a)))
    highBallAngle = math.degrees(math.atan((-b + math.sqrt(discriminant)) / (2 * a)))
...
def calculateTrajectoryDataToPoint(proj: Projectile.Projectile, xMax: float):
    """
    A procedure that takes in a projectile object and runs it through a drag-less projectile simulation, until point x.
